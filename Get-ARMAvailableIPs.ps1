...
.DESCRIPTION
    ...
    -VM NICs IP
    -ILB IPs
    -Orphan NICs that do not have VMs attached to it.
    -VPN Gateways with BGP Peering, get's the IP for the BGP Peering

    App Gateways to come.
...
.PARAMETER SourceSub
	-SubnetName : Name of the subnet that this script will be run against.
	
.PARAMETER SourceVNET
	--SubnetName : Name of the VNet that this script will be run against.

.EXAMPLE
        Get-AvailableIPsARM -SCOPE ALL --- Get's the IPs for every single VNET and Subnet in the subscription.
        Get-AvailableIPsARM -SCOPE SUBNET -SOURCESUB subnetname -SOURCEVNET Vnetname --- Get's the IPs for the specified VNET and Subnet in that subscription.

.NOTES
    Version 1.1
    AUTHOR: Henry Robalino, Microsoft Consulting Services
    LASTEDIT: June 24, 2016
#>
